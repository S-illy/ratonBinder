        ...
        public void ThemeControl(Control control)
        {
            ...
            if (control is Panel)
            {
                ...
                if (!(panel.Parent is TabControl) || !(panel.Parent is TableLayoutPanel))
                {
                    if (RoundedPanels)
                    {
                        SetRoundBorders(panel, 6, OScolors.Surface, 1);
                    }
                }
            }
            ...
            //}
            if (control.Name == "fastColoredTextBox1" || control.Name == "textBoxConsole")
            {
                control.GetType().GetProperty("BackColor")?.SetValue(control, OScolors.ControlLight);
                control.GetType().GetProperty("ForeColor")?.SetValue(control, OScolors.TextActive);
            }
